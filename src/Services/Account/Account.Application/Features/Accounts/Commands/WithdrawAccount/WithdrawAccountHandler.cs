...
public class WithdrawingHandler(IAccountRepository accountRepository, ILogger<WithdrawingHandler> logger)
    ...
    public async Task Handle(WithdrawAccountCommand request, CancellationToken cancellationToken)
    {
        ...
        {
            logger.LogWarning(
                $"Insufficient balance for AccountId: {request.AccountId}. Balance: {account.Balance}, Requested: {request.Amount}");
        ...
        logger.LogInformation(
            $"Withdrawal successful for AccountId: {request.AccountId}. New Balance: {account.Balance}");
    }
    }